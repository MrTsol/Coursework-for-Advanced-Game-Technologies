...
const float PhysicsSystem::UNIT_MULTIPLIER = 1.0f; //New
...
PhysicsSystem::PhysicsSystem(GameWorld& g) : gameWorld(g)	{
	...
	SetGravity(Vector3(0.0f, -1000.0f, 0.0f));
}
	...
	for (auto i = first; i != last; ++i)
	{
		...
		for (auto j = i + 1; j != last; ++j)
		{
			...
			if (CollisionDetection::ObjectIntersection(*i, *j, info))
			{
				...
				ImpulseResolveCollision(*info.a, *info.b, info.point); //Tutorial 5
...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const
{
	...
	if (Vector3::Dot(contactVelocity, p.normal) > 0)//New
	{												//New
		return;										//New
	}												//New
...
void PhysicsSystem::BroadPhase() {
	...
	QuadTree<GameObject*> tree(Vector2(1500, 1500), 7, 6);
...
void PhysicsSystem::NarrowPhase() {
	for (std::set<CollisionDetection::CollisionInfo>::iterator i = broadphaseCollisions.begin(); i != broadphaseCollisions.end(); ++i) {
		...
		if (CollisionDetection::ObjectIntersection(info.a, info.b, info)) {
			...
			allCollisions.insert(info); //Insert into out main set
		}
	}
}
