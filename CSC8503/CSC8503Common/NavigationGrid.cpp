...
{
	//Need to work our which node 'from' sits in, and 'to' sits in
	int fromX = ((from.x - offset.x) / nodeSize);
	int fromZ = ((from.z - offset.z) / nodeSize);

	int toX = ((to.x - offset.x) / nodeSize);
	int toZ = ((to.z - offset.z) / nodeSize);
	...
	while (!openList.empty())
	{
		...
		if (currentBestNode == endNode) //We have found the path!
		{
		...
		else
		{
			for (int i = 0; i < 4; ++i)
			{
				...
				if (!inOpen) //First time we have seen this neighbour..
				{
