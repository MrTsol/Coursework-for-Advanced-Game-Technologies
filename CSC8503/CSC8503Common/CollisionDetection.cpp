...
bool RayBoxIntersection(const Ray&r, const Vector3& boxPos, const Vector3& boxSize, RayCollision& collision)
{
	...
	//The rayDir loop above assumes that the rey is infinite both backwards and forwards, so we can intersect with objects 'behind' us!
...
bool CollisionDetection::RayAABBIntersection(const Ray&r, const Transform& worldTransform, const AABBVolume& volume, RayCollision& collision)
{
	...
	//Trying Stuff
	/*Vector3 worldTransform1212 = worldTransform.GetWorldPosition();
	Vector3 size3333 = volume.GetHalfDimensions();

	return RaySphereIntersection(r, worldTransform1212, size3333, collision);*/
	//---------------------
}
...
bool CollisionDetection::RayOBBIntersection(const Ray&r, const Transform& worldTransform, const OBBVolume& volume, RayCollision& collision)
{
	...
	//TRYING STUFF
	/*bool collided = RaySphereIntersection(tempRay, Vector3(), volume, collision);
	...
	const SphereVolume& volumeB, const Transform& worldTransformB, CollisionInfo& collisionInfo)
{
	...
	if (distance < volumeB.GetRadius()) //Yes, we are colliding!
	{
		collisionInfo.AddContactPoint(transform * closestPointOnBox + worldTransformA.GetWorldPosition(), distance == 0.0f ? delta.Normalised() : transform * localPoint.Normalised(), (volumeB.GetRadius() - distance));
