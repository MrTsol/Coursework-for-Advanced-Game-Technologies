...
void PositionConstraint::UpdateConstraint(float dt)	{
	...
	if (abs(offset) > 0.0f)
	{
		...
		if (constraintMass > 0.0f)
		{
			...
			float biasDefault = 0.08f;
			float bias = -(biasDefault / dt) * offset;

			float lambda = -(velocityDot + bias) / constraintMass;

			Vector3 aImpulse = offsetDir * lambda;
			Vector3 bImplulse = -offsetDir * lambda;

			physA->ApplyLinearImpulse(aImpulse); //Multiplied by mass here
			physB->ApplyLinearImpulse(bImplulse); //And here
			physA->SetAngularVelocity(Vector3(0.f, 0.f, 0.f));
			physB->SetAngularVelocity(Vector3(0.f, 0.f, 0.f));
		}
	}
}
